...
Este é o repositório `YanWaltrick/ESTOQUE-1.0` — o remote `origin` aponta para ele.
O desenvolvimento que antes acontecia em um fork (`caique-az`) já foi **integrado via
merge**; não há mais distinção fork/upstream. Os Pull Requests internos têm como base
a branch `main`.
