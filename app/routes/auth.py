...
from app.auth.security import PasswordValidator, validate_email
...
def _enviar_emails_senha_esquecida(user: User, mensagem_texto: str) -> tuple[bool, str]:
    ...
    try:
        ...
        if usuario_eh_email:
            ...
            msg_usuario = Message(
                subject=assunto_usuario, recipients=[user.username], body=corpo_usuario
            )
...
def login():
    ...
    if request.method == "POST":
        ...
        if not user.pode_tentar_login():
            ...
            flash(
                f"Sua conta foi bloqueada temporariamente por múltiplas tentativas falhas. Tente novamente em {minutos} minuto(s).",
                "error",
            )
            return render_template("login.html")
...
def perfil():
    ...
    if request.method == "POST":
        ...
        if username != current_user.username or not check_password_hash(
    ...
    if session.get("perfil_verified"):
        return render_template(
            "profile.html", usuario=current_user.username, foto_perfil=current_user.foto_perfil
        )
...
def perfil_password():
    ...
    if login_name != current_user.username:
        ...
        registrar_evento(
            ...
            descricao="Tentativa de mudança de senha com username diferente",
    ...
    if not PasswordValidator.verify_password(senha_atual, current_user.password):
        ...
        registrar_evento(
            tipo_evento="tentativa_mudanca_senha_falha",
            descricao="Tentativa com senha atual incorreta",
            usuario_responsavel=current_user.username,
        )
        return redirect(url_for("auth.perfil_senha"))
...
def forgot_password():
    if request.method == "POST":
        ...
        flash(
            "Se o usuário informado existir, um chamado foi registrado e um administrador irá analisar em breve.",
            "success",
        )
        return redirect(url_for("auth.forgot_password"))
