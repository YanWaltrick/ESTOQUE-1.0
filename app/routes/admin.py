...
from app.auth import require_permission, require_role
...
def editar_usuario(user_id):
    ...
    if request.method == "POST":
        ...
        pj_contratante_cnpj = request.form.get(
            "pj_contratante_cnpj", usuario.pj_contratante_cnpj
        ).strip()
        pj_contratante_endereco = request.form.get(
            "pj_contratante_endereco", usuario.pj_contratante_endereco
        ).strip()
        pj_contratada = request.form.get("pj_contratada", usuario.pj_contratada).strip()
        pj_contratada_cnpj = request.form.get(
            "pj_contratada_cnpj", usuario.pj_contratada_cnpj
        ).strip()
        ...
        if username:
            existing_username = User.query.filter(
                User.username == username, User.id != usuario.id
            ).first()
...
def upload_usuario_foto(user_id):
    ...
    try:
        ...
        db.session.rollback()
        return jsonify(
            {"success": False, "erro": f"Erro ao salvar imagem de perfil: {str(e)}"}
        ), 500
...
def deletar_usuario(user_id):
    ...
    if usuario.role == "admin":
        ...
        if admin_count <= 1:
            return jsonify(
                {"success": False, "message": "Não é possível deletar o último administrador."}
            ), 403
...
def toggle_bloqueio_usuario(user_id):
    ...
    if usuario.id == current_user.id:
        return jsonify(
            {"success": False, "message": "Você não pode bloquear a sua própria conta."}
        ), 403
...
def upload_documento_usuario(user_id):
    ...
    if extensao not in EXTENSOES_PERMITIDAS:
        return jsonify(
            {
                "success": False,
                "message": f"Tipo de arquivo não permitido. Extensões aceitas: {', '.join(EXTENSOES_PERMITIDAS)}",
            }
        ), 400
    ...
    if tamanho > TAMANHO_MAXIMO:
        return jsonify(
            {
                "success": False,
                "message": f"Arquivo muito grande. Máximo: 10MB. Seu arquivo: {tamanho / (1024 * 1024):.2f}MB",
            }
        ), 400
...
def _arquivo_em_disco_utilizavel(caminho):
    """True se o arquivo existe no disco e não está vazio.

    Um arquivo de 0 byte é tratado como ausente (sintoma de disco efêmero
    reciclado/parcial), forçando o fallback ao blob íntegro do banco em vez de
    servir conteúdo vazio.
    """
    try:
...
def _servir_documento(documento, as_attachment, download_name=None):
    """Serve o documento do disco e, se ausente/vazio/ilegível, cai para o blob no banco.

    Retorna uma Response do Flask, ou ``None`` quando o arquivo não existe em
    nenhuma das fontes (o chamador converte em 404). Falhas de leitura do disco
    (arquivo some entre o check e o envio) fazem fallback para o banco em vez de
    estourar erro.
    """
    caminho_arquivo = _caminho_documento(documento)

    if _arquivo_em_disco_utilizavel(caminho_arquivo):
        try:
            if as_attachment:
                return send_file(
                    caminho_arquivo,
            ...
            # Preview: deixa o Flask inferir o mimetype pela extensão do arquivo.
            return send_file(caminho_arquivo, as_attachment=False)
        except OSError:
            # Arquivo sumiu/ficou ilegível entre o check e o envio (disco efêmero):
            # segue para o fallback no banco.
            pass
...
def visualizar_documento(documento_id):
    ...
    try:
        resposta = _servir_documento(documento, as_attachment=False)
    except Exception:
        current_app.logger.exception("Erro ao visualizar documento %s", documento_id)
        return jsonify({"success": False, "message": "Erro ao abrir o documento."}), 500
    ...
    try:
        ...
        if documento.tipo_arquivo.lower() == "pdf" and documento.usuario:
            ...
            if doc_name.startswith("aditivo ao termo"):
                download_name = (
                    f"Aditivo ao Termo de Responsabilidade de {documento.usuario.username}.pdf"
                )
        ...
        current_app.logger.exception("Erro ao fazer download do documento %s", documento_id)
        return jsonify({"success": False, "message": "Erro ao fazer download do documento."}), 500
...
def listar_documentos_usuario(user_id):
    ...
    usuario = User.query.get_or_404(user_id)
    documentos = (
        DocumentoUsuario.query.filter_by(id_usuario=user_id)
    ...
    for doc in documentos:
        ...
        documento["download_url"] = url_for(
            "admin.download_documento", documento_id=doc.id_documento
        )
        documento["preview_url"] = url_for(
            "admin.visualizar_documento", documento_id=doc.id_documento
        )
        documento["pode_visualizar"] = doc.tipo_arquivo.lower() in {
            "pdf",
            "jpg",
            "jpeg",
            "png",
    ...
    return jsonify({"success": True, "usuario": usuario.username, "documentos": documentos_json})
...
def listar_itens_recebidos(user_id):
    ...
    itens_entrada = (
        ItemRecebido.query.filter_by(id_usuario=user_id, tipo_recebimento="entrada")
        .order_by(ItemRecebido.data_criacao.desc())
        .all()
    )

    itens_posteriormente = (
        ItemRecebido.query.filter_by(id_usuario=user_id, tipo_recebimento="posteriormente")
        .order_by(ItemRecebido.data_criacao.desc())
        .all()
    )
...
def gerar_relatorio_itens_usuario(user_id):
    ...
    itens_entrada = (
        ItemRecebido.query.filter_by(id_usuario=user_id, tipo_recebimento="entrada")
        .order_by(ItemRecebido.data_criacao.asc())
        .all()
    )
    ...
    try:
        ...
        return jsonify(
            {
                "success": False,
                "message": "Dependência reportlab não instalada. Rode: pip install reportlab",
            }
        ), 500
    ...
    def draw_wrapped_text(
        text, x, current_y, max_width, font_name="Times-Roman", font_size=10, leading=13
    ):
    ...
    def draw_items_table(current_y, itens):
        ...
        for row in table_data[1:]:
            ...
            for col_idx, cell in enumerate(row):
                # Desenhar célula
                pdf.rect(
                    col_x, data_y - row_height, col_widths[col_idx], row_height, fill=0, stroke=1
                )
                # Desenhar texto
    ...
    data_admissao = (
        usuario.data_admissao.strftime("%d/%m/%Y")
        if getattr(usuario, "data_admissao", None)
        else ""
    )
...
def adicionar_item_recebido(user_id):
    ...
    return jsonify(
        {"success": True, "message": "Item adicionado com sucesso!", "item": novo_item.to_dict()}
    )
...
def editar_item_recebido(item_id):
    ...
    return jsonify(
        {"success": True, "message": "Item atualizado com sucesso!", "item": item.to_dict()}
    )
...
def listar_termo_entrega(user_id):
    ...
    if not termo:
        termo_data = {
            ...
            "data_admissao": usuario.data_admissao.strftime("%Y-%m-%d")
            ...
            "pj_data_contrato": usuario.pj_data_contrato.strftime("%Y-%m-%d")
            if usuario.pj_data_contrato
            else None,
            "equipamentos": [],
            "data_criacao": None,
        ...
        return jsonify({"success": True, "termo": termo_data})

    termo_data = termo.to_dict()
    # Campo de input type="date" precisa de formato YYYY-MM-DD.
    termo_data["data_admissao"] = (
        termo.data_admissao.strftime("%Y-%m-%d") if termo.data_admissao else ""
    )
    ...
    termo_data["pj_contratante_cnpj"] = (
        termo.pj_contratante_cnpj or usuario.pj_contratante_cnpj or ""
    )
    termo_data["pj_contratante_endereco"] = (
        termo.pj_contratante_endereco or usuario.pj_contratante_endereco or ""
    )
    ...
    termo_data["pj_data_contrato"] = (
        termo.pj_data_contrato.strftime("%Y-%m-%d")
        if termo.pj_data_contrato
        else (usuario.pj_data_contrato.strftime("%Y-%m-%d") if usuario.pj_data_contrato else "")
    )
...
def atualizar_termo_entrega(user_id):
    ...
    return jsonify(
        {"success": True, "message": "Termo atualizado com sucesso!", "termo": termo.to_dict()}
    )
...
def adicionar_equipamento_termo(user_id):
    ...
    if not descricao:
        return jsonify(
            {"success": False, "message": "Descrição do equipamento é obrigatória."}
        ), 400
    ...
    ids_existentes = [
        ...
        if isinstance(eq, dict) and isinstance(eq.get("id"), int)
    ]
    ...
    try:
        ...
        for indice, arquivo in enumerate(arquivos):
            if arquivo and arquivo.filename:
                ...
                titulo_foto = (
                    titulos_fotos[indice].strip()
                    if indice < len(titulos_fotos) and titulos_fotos[indice]
                    else f"Foto {indice + 1}"
                )
                saved_fotos.append(
                    {
                        "arquivo": unique_name,
    ...
    registrar_evento(
        ...
        + (" (será em ADITIVO)" if criar_aditivo else ""),
        usuario_responsavel=current_user.username,
    )
    ...
    return jsonify(
        {
            ...
            + (" Um ADITIVO será criado na próxima geração." if criar_aditivo else ""),
...
@admin_bp.route(
    "/usuarios/<int:user_id>/termo-entrega/equipamentos/<int:eq_id>/deletar", methods=["DELETE"]
)
def deletar_equipamento_termo(user_id, eq_id):
    ...
    if not termo:
        return jsonify(
            {"success": False, "message": "Nenhum termo encontrado para este usuário."}
        ), 404
    ...
    try:
        ...
        if eh_aditivo:
            equipamentos_aditivo = [
                eq for eq in equipamentos if eq.get("tipo_documento") == "aditivo"
            ]
        ...
        for eq in equipamentos:
            ...
            item_existe = ItemRecebido.query.filter_by(
                id_usuario=user_id, descricao_item=descricao
            ).first()
        ...
        return jsonify(
            {"success": True, "message": mensagem, "documento_id": novo_doc.id_documento}
        )
