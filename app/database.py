...
def create_app():
    ...
    if not secret and os.getenv("FLASK_ENV") != "development":
        raise RuntimeError(
            "SECRET_KEY não definida. Configure SECRET_KEY em variáveis de ambiente."
        )
    ...
    app.config["SESSION_COOKIE_SECURE"] = (
        False if is_dev else os.getenv("SESSION_COOKIE_SECURE", "True").lower() == "true"
    )
    ...
    app.config["SESSION_COOKIE_SAMESITE"] = os.getenv(
        "SESSION_COOKIE_SAMESITE", "None" if is_dev else "Lax"
    )
    ...
    app.config["POWER_AUTOMATE_TIMEOUT_SECONDS"] = int(
        os.getenv("POWER_AUTOMATE_TIMEOUT_SECONDS", 10)
    )
