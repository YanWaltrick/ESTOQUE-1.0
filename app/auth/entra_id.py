...
class EntraIDConfig:
    ...
    def __init__(self):
        ...
        self.authority = (
            f"https://login.microsoftonline.com/{self.tenant_id}" if self.tenant_id else None
        )
...
class EntraIDClient:
    ...
    def __init__(self, config: EntraIDConfig, session_store: dict | None = None):
        """
    ...
    def validate_token(
        self, code: str, redirect_uri: str, session_state: str | None = None
    ) -> dict | None:
        """
        ...
        try:
            ...
            result = self.app.acquire_token_by_authorization_code(
                code=code, scopes=["User.Read"], redirect_uri=redirect_uri
            )
    ...
    def extract_user_info(self, token_result: dict) -> dict | None:
        """
        ...
        try:
            ...
            if "id_token_claims" in token_result:
                ...
                if user_info["email"] and user_info["oid"]:
                    logger.info(
                        f"Informações extraídas: email={user_info['email']}, oid={user_info['oid']}"
                    )
                    return user_info
    ...
    def _get_me_endpoint(self, access_token: str) -> dict | None:
        """
...
def create_entra_client(session_store: dict | None = None) -> EntraIDClient | None:
    """
