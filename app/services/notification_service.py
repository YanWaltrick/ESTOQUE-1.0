...
def _montar_payload_legacy(chamada, evento: str, status_anterior: str | None = None) -> dict:
    ...
    return {
        ...
        'chamada': {
            ...
            'foto_url': f'/static/uploads/chamadas/{chamada.foto_anexo}' if chamada.foto_anexo else None,
            'usuario': {
                ...
                'localizacao': usuario.localizacao if usuario and getattr(usuario, 'localizacao', None) else None,
                'foto_perfil': usuario.foto_perfil if usuario and getattr(usuario, 'foto_perfil', None) else None,
            },
        },
        'destinatarios': {
...
def montar_payload_notificacao_chamada(chamada, evento: str, status_anterior: str | None = None, modo: str | None = None) -> dict:
    if modo == 'legacy':
...
def enviar_notificacao_chamada(chamada, evento: str, status_anterior: str | None = None) -> tuple[bool, str]:
    url_webhook, modo = _obter_destino_webhook()
    if not url_webhook:
        return True, ''

    payload = montar_payload_notificacao_chamada(chamada, evento, status_anterior=status_anterior, modo=modo)
    ...
    try:
        ...
        current_app.logger.warning('Falha ao enviar notificação para webhook do Teams: %s', mensagem)
