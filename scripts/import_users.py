    ...
    if filepath_lower.endswith('.xls'):
        ...
        for row_idx in range(header_row + 1, sheet.nrows):
            ...
            if any(str(v).strip() for v in row_values):
                rows.append({headers[i] if i < len(headers) else f'col_{i}': row_values[i] for i in range(len(row_values))})
        return rows
        ...
        headers = [normalize_text(v) for v in next(sheet.iter_rows(min_row=header_row, max_row=header_row, values_only=True))]
        rows = []
...
def import_users(filepath):
    ...
    tipo_idx = map_field(headers, ['tipo vínculo', 'tipo vinculo', 'tipo contrato', 'vínculo', 'vinculo'])
    funcao_idx = map_field(headers, ['função', 'funcao', 'cargo', 'role'])
    admissao_idx = map_field(headers, ['admissão', 'admissao', 'data_admissao', 'data admissao', 'hire_date'])
    ...
    with app.app_context():
        ...
        existing_by_cpf = {normalize_cpf(user.cpf): user for user in existing_users if normalize_cpf(user.cpf)}
        ...
        for row_number, row in enumerate(rows, start=1):
            ...
            tipo_vinculo = normalize_text(row.get(headers[tipo_idx], '')) if tipo_idx is not None else 'CLT'
            funcao = normalize_text(row.get(headers[funcao_idx], '')) if funcao_idx is not None else ''
            ...
            if user is None:
                ...
                user.tipo_contrato = 'PJ' if ('pj' in tipo_vinculo.lower() or 'pessoa juridica' in tipo_vinculo.lower() or 'pessoa jurídica' in tipo_vinculo.lower()) else 'CLT'
